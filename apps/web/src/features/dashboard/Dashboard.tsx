...
function OpportunityTable({ items, onEvidence }: { items: Opportunity[]; onEvidence: (evidence: EvidenceRef) => void }) {
  ...
  return (
    <section className="opportunity-section">
      ...
      <div className="opportunity-table" role="table" aria-label="Öncelikli fırsatlar">
        ...
        <button className="table-more" type="button">Tüm fırsatları gör <ArrowRight size={17} /></button>
      </div>
    </section>
  );
}
...
function PlanRail({ plan }: { plan: GrowthDiagnostic["plan"] }) {
  return (
    <aside className="plan-rail">
      ...
      <button type="button">Planın tamamını gör <ArrowRight size={17} /></button>
    </aside>
  );
}
  ...
  return (
    <div className="drawer-backdrop" role="presentation" onMouseDown={onClose}>
      <aside className="evidence-drawer" role="dialog" aria-modal="true" aria-label="Kanıt ayrıntısı" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <button className="primary-button" type="button">Kaynak satırını aç <ExternalLink size={16} /></button>
      </aside>
    </div>
  );
}

function LowerActivity({ diagnostic, onEvidence }: { diagnostic: GrowthDiagnostic; onEvidence: (item: EvidenceRef) => void }) {
  const evidence = diagnostic.opportunities.flatMap((item) => item.evidence).slice(0, 4);
  return (
    <div className="lower-activity">
      <section>
        <header><h2>Kanıt ve kaynaklar</h2><button type="button">Tümünü gör</button></header>
      ...
      <section>
        <header><h2>Onay ve aktiviteler</h2><button type="button">Tümünü gör</button></header>
        <div className="activity-row"><ClipboardCheck size={21} /><span><strong>Predictive bakım yazılımı iş vakası</strong><small>Onay: Finans Direktörü</small></span><em className="tag tag--amber">Onay bekliyor</em></div>
        <div className="activity-row"><FileText size={21} /><span><strong>OEM ihracat paketi kapsamı güncellendi</strong><small>Aktivite: Strateji Ekibi</small></span><em className="tag">Taslak</em></div>
        <div className="activity-row"><ShieldCheck size={21} /><span><strong>Enerji retrofit paketi incelendi</strong><small>Onay: Operasyon Direktörü</small></span><em className="tag tag--green">İncelendi</em></div>
      </section>
    </div>
  );
}
  ...
  return (
    <main className="dashboard-page">
      ...
      <div className="dashboard-grid">
        ...
        <OpportunityTable items={diagnostic.opportunities} onEvidence={setSelectedEvidence} />
