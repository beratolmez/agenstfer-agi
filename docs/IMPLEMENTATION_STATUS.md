...
- **Phase 22 (Completed Round 2 Kapsamlı Teknik Denetim ve Çözüm - ADR-0024)**: Included `growth-diagnostic` in LangGraph StateGraph engine selection, removed deprecated `settings.enable_dbos` from approval decision endpoints, updated `docker-compose.model-download.yml` with standalone Ollama configuration, aligned `infra/aws/docker-compose.prod.yml` with `AGI_CLOUD_*` env vars and proxy config, added `web-proxy` to `NO_PROXY` in `docker-compose.cloud.yml`, and cleaned legacy `apps/services/` references in `pyproject.toml` and `Dockerfile`.
