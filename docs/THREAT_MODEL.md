...
1. React Browser UI to Nginx/FastAPI backend.
2. FastAPI to PostgreSQL, raw vault, Git bundle, and ChromaDB.
3. FastAPI to Gemini API through the egress gateway.
...
6. AWS control plane to Gemini API or outbound inference gateway.
...
- Internal Docker network; only ingress proxy publishes a host port for the React UI.
- Access to the Gemini API is managed via explicit egress configurations and API key policies, scoped down to required domains.
...
- Prompt-injection separation: source content cannot modify system policy or tool scope. The FastAPI
...
- **Public inference endpoint:** Access to Gemini API uses the selected private VPC outbound proxy or gateway. Backend API ports must not be exposed to the public.
- **Cross-customer API quota leakage:** the MVP prefers a dedicated Gemini API configuration and quota per customer. Shared API credentials require tenant-aware routing, encrypted job/result handling, quota isolation, noisy-neighbor controls, and a new review.
...
- **Dynamic worker escalation:** a bounded task orchestrator can select only code-defined workers,
  tools, budgets, rounds, and data scopes via Pydantic AI. It cannot create arbitrary workers, call unrestricted MCP,
...
- Classification and redaction before cloud calls. Because the MVP diagnostic computes aggregate
  ...
  the Gemini API call before prompt transmission. The final model-gateway boundary also reapplies contact
...
- Compose allowlists application environment variables. Gemini API keys are mounted only as the
...
- Cancellation changes application/approval/candidate state only after LangGraph confirms cancellation;
...
Gemini API tests must also cover aggregate-scope classification propagation, trust-boundary mismatch,
